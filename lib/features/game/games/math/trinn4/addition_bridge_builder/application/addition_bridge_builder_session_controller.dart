...
class AdditionBridgeBuilderSessionController {
  final List<AdditionBridgeBuilderQuestion> _questions;
  int _index = 0;
  ...
  AdditionBridgeBuilderSessionController({
    AdditionBridgeBuilderEngine? engine,
    int roundCount = 8,
  }) : _questions = (engine ?? AdditionBridgeBuilderEngine()).createQuestions(
         count: roundCount < 1 ? 1 : roundCount,
       );

  AdditionBridgeBuilderQuestion get currentQuestion => _questions[_index];
  ...
  GameResult buildResult() {
    ...
    final pointsEarned = (_correctAnswers * 14) + (_bestStreak * 5);
