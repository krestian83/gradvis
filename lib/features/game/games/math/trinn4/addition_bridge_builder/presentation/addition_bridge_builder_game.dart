...
import '../application/addition_bridge_builder_session_controller.dart';
import '../domain/addition_bridge_builder_engine.dart';

class AdditionBridgeBuilderGame extends StatefulWidget implements GameWidget {
  @override
  final ValueChanged<GameResult> onComplete;

  const AdditionBridgeBuilderGame({super.key, required this.onComplete});

  @override
  State<AdditionBridgeBuilderGame> createState() =>
      _AdditionBridgeBuilderGameState();
}

class _AdditionBridgeBuilderGameState extends State<AdditionBridgeBuilderGame> {
  final AdditionBridgeBuilderSessionController _session =
      AdditionBridgeBuilderSessionController();
  MathHelpController? _mathHelpController;
  String _feedback = 'Bygg bro over tierne og finn summen.';
  GameResult? _result;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'To tall med tier-overgang i hodet.',
  ...
  void _submitAnswer(AdditionBridgeBuilderQuestion question, int answer) {
    if (_session.isFinished || _completed) {
      return;
    }
    ...
    setState(() {
      _feedback = isCorrect
          ? 'Riktig! Bruk samme strategi videre.'
