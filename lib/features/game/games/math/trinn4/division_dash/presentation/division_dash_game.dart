...
import '../application/division_dash_session_controller.dart';
import '../domain/division_dash_engine.dart';

class DivisionDashGame extends StatefulWidget implements GameWidget {
  @override
  final ValueChanged<GameResult> onComplete;

  const DivisionDashGame({super.key, required this.onComplete});

  @override
  State<DivisionDashGame> createState() => _DivisionDashGameState();
}

class _DivisionDashGameState extends State<DivisionDashGame> {
  final DivisionDashSessionController _session =
      DivisionDashSessionController();
  MathHelpController? _mathHelpController;
  String _feedback = 'Del tallene riktig.';
  GameResult? _result;
  ...
  void _publishMathHelpContext() {
    ...
    _mathHelpController?.setContext(
      MathHelpContext(
        ...
        operation: 'division',
        operands: [question.dividend, question.divisor],
  ...
  void _submitAnswer(DivisionDashQuestion question, int answer) {
    if (_session.isFinished || _completed) {
      return;
    }
