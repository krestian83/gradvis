...
import '../application/multiplication_table_sprint_session_controller.dart';
import '../domain/multiplication_table_sprint_engine.dart';

class MultiplicationTableSprintGame extends StatefulWidget
    implements GameWidget {
  @override
  final ValueChanged<GameResult> onComplete;

  const MultiplicationTableSprintGame({super.key, required this.onComplete});

  @override
  State<MultiplicationTableSprintGame> createState() =>
      _MultiplicationTableSprintGameState();
}

class _MultiplicationTableSprintGameState
    extends State<MultiplicationTableSprintGame> {
  final MultiplicationTableSprintSessionController _session =
      MultiplicationTableSprintSessionController();
  MathHelpController? _mathHelpController;
  String _feedback = 'Finn produktet raskt i hodet.';
  GameResult? _result;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Tabellene 6-10 med varierte oppgaver.',
  ...
  void _publishMathHelpContext() {
    ...
    _mathHelpController?.setContext(
      MathHelpContext(
        ...
        operands: [question.leftFactor, question.rightFactor],
  ...
  void _submitAnswer(MultiplicationTableSprintQuestion question, int answer) {
    if (_session.isFinished || _completed) {
      return;
    }
