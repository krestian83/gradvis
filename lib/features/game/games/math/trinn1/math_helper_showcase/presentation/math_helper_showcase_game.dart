...
import '../application/math_helper_showcase_session_controller.dart';
...
class MathHelperShowcaseGame extends StatefulWidget implements GameWidget {
  @override
  final ValueChanged<GameResult> onComplete;

  final Duration feedbackDelay;
  final MathHelperShowcaseSessionController? sessionController;

  const MathHelperShowcaseGame({
    super.key,
    required this.onComplete,
    this.feedbackDelay = const Duration(milliseconds: 380),
    this.sessionController,
  });

  @override
  State<MathHelperShowcaseGame> createState() => _MathHelperShowcaseGameState();
}

class _MathHelperShowcaseGameState extends State<MathHelperShowcaseGame> {
  late final MathHelperShowcaseSessionController _session;
  ...
  void initState() {
    super.initState();
    _session =
        widget.sessionController ?? MathHelperShowcaseSessionController();
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Math helper showcase', style: theme.textTheme.headlineSmall),
          const SizedBox(height: 6),
          Text(
            'Denne levelen viser visualiseringer uten regneartene. '
            'Bruk hjelp-knappen oppe til hoyre.',
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  Text(
                    _topicLabel(question.topicFamily),
          ...
          Expanded(
            child: GridView.count(
              ...
              children: options.map((option) {
                ...
                return FilledButton(
                  key: ValueKey('math-helper-showcase-option-$option'),
          ...
          const SizedBox(height: 8),
          Text(
            'Riktige svar: ${_session.correctAnswers}',
  ...
  void _queueMathHelpContextPublish() {
    if (_isPublishingHelpContext || _hasCompleted) return;
  ...
  String _topicLabel(ShowcaseTopicFamily topicFamily) {
    return switch (topicFamily) {
      ShowcaseTopicFamily.geometry => 'Tema: Geometri',
      ShowcaseTopicFamily.measurement => 'Tema: Maling',
      ShowcaseTopicFamily.algorithmicThinking => 'Tema: Algoritmisk tenkning',
    };
  }
