...
import '../domain/subtraction_quiz_engine.dart';

class SubtractionQuizReward {
  final int stars;
  final int points;

  const SubtractionQuizReward({required this.stars, required this.points});
}

class SubtractionQuizSessionController {
  final SubtractionQuizEngine _engine;
  final List<SubtractionQuizQuestion> _questions;

  int _currentIndex = 0;
  int _correctCount = 0;

  SubtractionQuizSessionController({SubtractionQuizEngine? engine})
    : this._(engine ?? const SubtractionQuizEngine());

  SubtractionQuizSessionController._(this._engine)
  ...
  SubtractionQuizQuestion get currentQuestion => _questions[_currentIndex];
  ...
  SubtractionQuizReward get reward {
    final stars = _starsFromAccuracy();
    final points = _correctCount * 3 + totalRounds;
    return SubtractionQuizReward(stars: stars, points: points);
  }
