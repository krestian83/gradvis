...
import '../application/subtraction_quiz_session_controller.dart';

class SubtractionQuizGame extends StatefulWidget implements GameWidget {
  @override
  final ValueChanged<GameResult> onComplete;

  final Duration feedbackDelay;
  final SubtractionQuizSessionController? sessionController;

  const SubtractionQuizGame({
    super.key,
  ...
  @override
  State<SubtractionQuizGame> createState() => _SubtractionQuizGameState();
}

class _SubtractionQuizGameState extends State<SubtractionQuizGame> {
  late final SubtractionQuizSessionController _session;
  ...
  void initState() {
    super.initState();
    _session = widget.sessionController ?? SubtractionQuizSessionController();
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Subtraksjon', style: theme.textTheme.headlineSmall),
          const SizedBox(height: 6),
          Text(
            'Los oppgaven. Bruk hjelp-knappen oppe til hoyre '
            'for visualisering.',
          ...
          Expanded(
            child: GridView.count(
              ...
              children: options.map((option) {
                ...
                return FilledButton(
                  key: ValueKey('subtraction-quiz-option-$option'),
