...
import '../application/addition_quiz_session_controller.dart';

class AdditionQuizGame extends StatefulWidget implements GameWidget {
  @override
  final ValueChanged<GameResult> onComplete;

  final Duration feedbackDelay;
  final AdditionQuizSessionController? sessionController;

  const AdditionQuizGame({
    super.key,
  ...
  @override
  State<AdditionQuizGame> createState() => _AdditionQuizGameState();
}

class _AdditionQuizGameState extends State<AdditionQuizGame> {
  late final AdditionQuizSessionController _session;
  ...
  void initState() {
    super.initState();
    _session = widget.sessionController ?? AdditionQuizSessionController();
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Addisjon', style: theme.textTheme.headlineSmall),
          const SizedBox(height: 6),
          Text(
            'Løs oppgaven. Bruk hjelp-knappen oppe til hoyre '
            'for visualisering.',
          ...
          Expanded(
            child: GridView.count(
              ...
              children: options.map((option) {
                ...
                return FilledButton(
                  key: ValueKey('addition-quiz-option-$option'),
