...
import '../domain/addition_quiz_engine.dart';

class AdditionQuizReward {
  final int stars;
  final int points;

  const AdditionQuizReward({required this.stars, required this.points});
}

class AdditionQuizSessionController {
  final AdditionQuizEngine _engine;
  final List<AdditionQuizQuestion> _questions;

  int _currentIndex = 0;
  int _correctCount = 0;

  AdditionQuizSessionController({AdditionQuizEngine? engine})
    : this._(engine ?? const AdditionQuizEngine());

  AdditionQuizSessionController._(this._engine)
  ...
  AdditionQuizQuestion get currentQuestion => _questions[_currentIndex];
  ...
  AdditionQuizReward get reward {
    final stars = _starsFromAccuracy();
    final points = _correctCount * 3 + totalRounds;
    return AdditionQuizReward(stars: stars, points: points);
  }
