...
import '../domain/multiplication_quiz_trinn3_engine.dart';

class MultiplicationQuizTrinn3Reward {
  final int stars;
  final int points;

  const MultiplicationQuizTrinn3Reward({
    required this.stars,
    required this.points,
  });
}

class MultiplicationQuizTrinn3SessionController {
  final MultiplicationQuizTrinn3Engine _engine;
  final List<MultiplicationQuizTrinn3Question> _questions;

  int _currentIndex = 0;
  int _correctCount = 0;

  MultiplicationQuizTrinn3SessionController({
    MultiplicationQuizTrinn3Engine? engine,
  }) : this._(engine ?? const MultiplicationQuizTrinn3Engine());

  MultiplicationQuizTrinn3SessionController._(this._engine)
  ...
  MultiplicationQuizTrinn3Question get currentQuestion =>
  ...
  MultiplicationQuizTrinn3Reward get reward {
    final stars = _starsFromAccuracy();
    final points = _correctCount * 3 + totalRounds;
    return MultiplicationQuizTrinn3Reward(stars: stars, points: points);
  }
